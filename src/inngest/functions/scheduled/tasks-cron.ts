...
/**
 * Shared logic for processing scheduled tasks for all users.
 * Used by both the cron function and manual trigger function.
 * 
 * PROCESS:
 ...
 */
async function processScheduledTasks({ step, logger }: { step: any; logger: any }) {
...
/**
 * CRON-TRIGGERED scheduled tasks function.
 * Runs hourly at minute 0 to process all user scheduled tasks.
 * 
...
export const scheduledTasksCronFn = inngest.createFunction(
  { 
    ...
    // Concurrency limit prevents multiple executions
    concurrency: {
      limit: 1,
      key: "global"
    }
  },
  { cron: "0 * * * *" }, // Single cron trigger only
  async ({ step, logger }) => {
    logger.info('Starting scheduled tasks check (cron triggered)');
    return processScheduledTasks({ step, logger });
  }
);

/**
 * MANUALLY-TRIGGERED scheduled tasks function.
 * Can be triggered via POST /api/inngest/trigger-schedule for testing/debugging.
 * 
 * @returns Summary of triggered tasks
 */
export const scheduledTasksManualFn = inngest.createFunction(
  { id: "scheduled-tasks-manual" },
  { event: "scheduled-tasks/manual-trigger" },
  async ({ step, logger }) => {
    logger.info('Starting scheduled tasks check (manually triggered)');
    return processScheduledTasks({ step, logger });
  }
);
