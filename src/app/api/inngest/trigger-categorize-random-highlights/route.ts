...
export async function POST(request: NextRequest) {
  try {
    ...
    await inngest.send({
      name: "automations/categorize-random-highlights",
      data: { userId }
    });
    
    return createSuccessResponse(
      { triggered: true }, 
      'Random highlight categorization automation triggered successfully'
    );
  } catch (error) {
    console.error('Error triggering random highlight categorization automation:', error);
